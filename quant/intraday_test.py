...
PARAMS = {
    "daily_expma": [5, 10, 20],
    "daily_divergence": 0.015,  # 1.5%
    "intraday_expma": [5, 13],
    "open_window": 6,  # 30 分钟
    "entry_bar": 7,
    "force_close_hour": 14,
    "force_close_minute": 57,
}
# ===========================
...
def check_direction(daily_klines, params):
    ...
    if latest_e5 > latest_e10 > latest_e20:
        ...
        if gap1 > params["daily_divergence"] and gap2 > params["daily_divergence"]:
            return 1, f"多 (间距{gap1*100:.1f}%/{gap2*100:.1f}%)"
    ...
    if latest_e5 < latest_e10 < latest_e20:
        ...
        if gap1 > params["daily_divergence"] and gap2 > params["daily_divergence"]:
            return -1, f"空 (间距{gap1*100:.1f}%/{gap2*100:.1f}%)"
    
    return 0, "震荡"


print("=" * 70)
print("日内 5 分钟均线策略 - 单品种测试")
print("=" * 70)
print(f"合约：{SYMBOL}")
print(f"时间：{START_DATE} 至 {END_DATE}")
print(f"参数：日线 EXPMA{PARAMS['daily_expma']}, 间距>{PARAMS['daily_divergence']*100}%")
print(f"      5 分钟 EXPMA{PARAMS['intraday_expma']}, 开仓第{PARAMS['entry_bar']}根 K 线")
print("=" * 70)
...
try:
    ...
    # 检查日线方向
    direction, direction_str = check_direction(daily_klines, PARAMS)
    print(f"\n日线状态：{direction_str}")
    ...
    while True:
        ...
        if api.is_changing(klines_5m):
            ...
            is_afternoon = time(13, 30) <= curr_time <= time(15, 0)
            
            # 强制平仓时间
            if is_afternoon and curr_time >= time(PARAMS["force_close_hour"], PARAMS["force_close_minute"]):
                if position != 0:
                    print(f"  [{curr_time}] 强制平仓 @ {curr_close:.0f}")
                    target_pos.set_target_volume(0)
                    trades.append({'time': curr_time, 'action': 'FORCE_CLOSE', 'price': curr_close})
                    position = 0
                continue
            ...
            # 第 7 根及之后：检查开仓
            if position == 0 and session_bars == PARAMS["open_window"]:
                # 检查突破
                if today_direction == 1:
                    ...
                    if curr_close > session_high:
                        ...
                        position = 1
                        trades.append({'time': curr_time, 'action': 'BUY', 'price': curr_close, 'stop_loss': stop_loss})
                    ...
                    if curr_close < session_low:
                        ...
                        entry_price = curr_close
                        position = -1
                        trades.append({'time': curr_time, 'action': 'SELL', 'price': curr_close, 'stop_loss': stop_loss})
            ...
            if position != 0:
                ...
                if position == 1 and curr_low <= stop_loss:
                    print(f"  [{curr_time}] 止损平仓 (多) @ {curr_close:.0f}")
                    target_pos.set_target_volume(0)
                    trades.append({'time': curr_time, 'action': 'STOP_LOSS', 'price': curr_close})
                    position = 0
                    continue
                
                if position == -1 and curr_high >= stop_loss:
                    print(f"  [{curr_time}] 止损平仓 (空) @ {curr_close:.0f}")
                    target_pos.set_target_volume(0)
                    trades.append({'time': curr_time, 'action': 'STOP_LOSS', 'price': curr_close})
                    position = 0
                    continue
                ...
                if len(klines_5m) >= PARAMS["intraday_expma"][1] + 2:
                    ...
                    if position == 1 and prev_e5 >= prev_e13 and curr_e5 < curr_e13:
                        print(f"  [{curr_time}] 止盈平仓 (多，死叉) @ {curr_close:.0f}")
                        target_pos.set_target_volume(0)
                        trades.append({'time': curr_time, 'action': 'TAKE_PROFIT', 'price': curr_close})
                        position = 0
                        continue
                    
                    # 空单：金叉平仓
                    if position == -1 and prev_e5 <= prev_e13 and curr_e5 > curr_e13:
                        print(f"  [{curr_time}] 止盈平仓 (空，金叉) @ {curr_close:.0f}")
                        target_pos.set_target_volume(0)
                        trades.append({'time': curr_time, 'action': 'TAKE_PROFIT', 'price': curr_close})
                        position = 0
                        continue
    ...
    print(f"  绝对收益：  {stats.get('balance', 0) - stats.get('init_balance', 0):>12,.2f} 元")
    ...
    if trades:
        ...
        for i, trade in enumerate(trades, 1):
            ...
            if action == 'BUY':
                print(f"  {i}. [{t}] 开多 @ {price:.0f}, 止损={trade.get('stop_loss', 'N/A'):.0f}")
            elif action == 'SELL':
                print(f"  {i}. [{t}] 开空 @ {price:.0f}, 止损={trade.get('stop_loss', 'N/A'):.0f}")
            elif action == 'STOP_LOSS':
                print(f"  {i}. [{t}] 止损平仓 @ {price:.0f}")
            elif action == 'TAKE_PROFIT':
                print(f"  {i}. [{t}] 止盈平仓 @ {price:.0f}")
            elif action == 'FORCE_CLOSE':
                print(f"  {i}. [{t}] 强制平仓 @ {price:.0f}")
