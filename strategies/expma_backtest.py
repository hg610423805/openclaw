...
class EXPMA_Backtest:
    def __init__(self, symbol, start_date, end_date, initial_capital=100000):
        ...
        self.api = TqApi(TqAuth(user_name="hg610423805", password="Hgssh285755"))
        
        print(f"[{datetime.datetime.now()}] 正在获取 {symbol} 历史数据...")
        self.klines_daily = self.api.get_kline_serial(symbol, 86400, 500)
        self.klines_5m = self.api.get_kline_serial(symbol, 300, 10000)
        
        start_ts = datetime.datetime.strptime(start_date, "%Y-%m-%d").timestamp() * 1e9
        end_ts = datetime.datetime.strptime(end_date, "%Y-%m-%d").timestamp() * 1e9
        
        self.klines_daily = self.klines_daily[
            (self.klines_daily['datetime'] >= start_ts) & 
            (self.klines_daily['datetime'] <= end_ts)
        ].reset_index(drop=True)
        
        print(f"[{datetime.datetime.now()}] 数据加载完成 - 日线：{len(self.klines_daily)}条")
    ...
    def get_6bar_range(self, idx):
        ...
        high = recent['highest'].max()
        low = recent['lowest'].min()
        return high, low
        ...
        for i in range(10, len(self.klines_5m)):
            ...
            if trend == 1:
                if self.position == 0 and curr_price > range_high:
                    ...
                    self.entry_bar_low = curr_kline['lowest']
                    self.trades.append({
                ...
                if self.position == 0 and curr_price < range_low:
                    self.position = -1
                    self.entry_price = curr_price
                    self.entry_bar_high = curr_kline['highest']
                    self.trades.append({
                        'open_time': curr_time.strftime("%Y-%m-%d %H:%M"),
                        'type': '开仓',
                        'direction': '空',
